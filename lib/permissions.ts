...
export async function seedSchoolModulesAndRolePerms(schoolId: string) {
  ...
  // Teachers
  for (const key of ["DASHBOARD", "STUDENTS", "ATTENDANCE", "TIMETABLE", "COMMUNICATION", "HOMEWORK", "PROGRESS_CARD", "EXAMS", "ACADEMICS", "GALLERY", "LEARNING_CENTER"] as const) {
  ...
  for (const key of ["DASHBOARD", "FEES", "REPORTS", "EXAMS", "ACADEMICS", "GALLERY", "LEARNING_CENTER", "YOUTUBE_LEARNING", "SCHOOL_CALENDAR"] as const) {
...
export async function getEffectivePermissions({
  ...
  if ((enabled.has("EXAMS") || examsNotConfigured) && !map.EXAMS) {
    if (role?.key === "ADMIN") map.EXAMS = "ADMIN";
    else if (role?.key === "HEAD_MASTER" || role?.key === "PRINCIPAL") map.EXAMS = "APPROVE";
    else if (role?.key === "TEACHER" || role?.key === "CLASS_TEACHER") map.EXAMS = "EDIT";
    else map.EXAMS = "VIEW";
  }
